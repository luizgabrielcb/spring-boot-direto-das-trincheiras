...
public class UserControllerRestAssuredIT extends IntegrationTestConfig {
    ...
    void findAll_ReturnsListWithAllUsers_WhenArgumentIsNull() {
        ...
        JsonAssertions.assertThatJson(response).and(user -> {
            user.node("[0].id").asNumber().isPositive();
            user.node("[1].id").asNumber().isPositive();
        });
    ...
    void findById_ReturnsUserById_WhenSuccessful() {
        ...
        String response = RestAssured.given()
                .contentType(ContentType.JSON).accept(ContentType.JSON)
                .pathParam("id", user.getFirst().getId())
                .when()
                .get(URL + "/{id}")
                .then()
                .statusCode(HttpStatus.OK.value())
    ...
    void findById_ThrowsNotFound_WhenUserIsNotFound() {
        ...
        RestAssured.given()
                ...
                .body(Matchers.equalTo(responseExpected))
                .log().all();
    }
    ...
    void delete_RemoveUser_WhenSuccessful() {
        ...
        RestAssured.given()
                .contentType(ContentType.JSON).accept(ContentType.JSON)
                .pathParam("id", userToDelete.getFirst().getId())
                .when()
                .delete(URL + "/{id}")
                .then()
                .statusCode(HttpStatus.NO_CONTENT.value())
